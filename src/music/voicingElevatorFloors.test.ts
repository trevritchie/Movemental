...
describe('voicingElevatorFloors', () => {
  ...
  it('pairs child and parent floors at each Every Other stop', () => {
    const pairs = EVERY_OTHER_CHILD_INPUT_STEPS.map((childSteps, i) => [
      TILT_VOICING_LEVEL_NAMES[childSteps],
      TILT_VOICING_LEVEL_NAMES[EVERY_OTHER_PARENT_INPUT_STEPS[i]!],
    ]);
  ...
  it('keeps parents on Third for the lowest two roll stops', () => {
    expect(everyOtherInputStepsFromRoll(-1, 'Wind')).toBe(1);
    expect(everyOtherInputStepsFromRoll(-0.75, 'Wind')).toBe(1);
    expect(everyOtherStopIndexFromRoll(-1)).toBe(0);
    expect(everyOtherStopIndexFromRoll(-0.75)).toBe(1);
  });
