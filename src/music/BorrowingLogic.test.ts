...
import { generatePitchStructure, getInitialBorrowingState, prepareVoicingInput, type BorrowingState, applyBorrowingOverlay, findNextLowerNote, getMutedPitchClasses, generatePitchStructureForVoicing, getVoicePitchClasses, findNextHigherNote, getRootPositionMapping, closestMidiWithPitchClass } from './BorrowingLogic';
...
describe('BorrowingLogic', () => {
  ...
  describe('findNextHigherNote', () => {
    it('returns the next higher pitch class in the same octave', () => {
      // C4 (60) -> D4 (62)
      expect(findNextHigherNote(60, firePitches)).toBe(62);
    });

    it('wraps to the lowest pitch class in the next octave when none higher', () => {
      // B4 (71) -> D4 in next octave (74)
      expect(findNextHigherNote(71, firePitches)).toBe(74);
    });
  });
  ...
  describe('getRootPositionMapping', () => {
    it('maps Branch at C by harmonic degree', () => {
      ...
      expect(getRootPositionMapping(branch)).toEqual({
        1: 0,
        2: 1,
        3: 2,
        4: 3,
    ...
    it('maps Brother Branch at C with root on dictionary first pitch', () => {
      ...
      expect(getRootPositionMapping(bro)).toEqual({
        1: 1,
        2: 2,
        3: 3,
        4: 0,
  ...
  describe('generatePitchStructure', () => {
    it('nulls exactly one slot when each Branch voice is muted', () => {
      const branch = branchChord();
      const allOn = generatePitchStructure(
        branch,
      ...
      expect(
        generatePitchStructure(branch, borrowingStateWithMutedVoice(1))
      ).toEqual([null, 4, 7, 9]);
      expect(
        generatePitchStructure(branch, borrowingStateWithMutedVoice(2))
      ).toEqual([0, null, 7, 9]);
      expect(
        generatePitchStructure(branch, borrowingStateWithMutedVoice(3))
      ).toEqual([0, 4, null, 9]);
      expect(
        generatePitchStructure(branch, borrowingStateWithMutedVoice(4))
      ).toEqual([0, 4, 7, null]);
    });
    ...
    it('skips borrowing on muted lines in pitch structure', () => {
      ...
      const structure = generatePitchStructure(branch, state);
    ...
    it('getMutedPitchClasses mutes natural and borrowed PC when line is off', () => {
      ...
      const muted = getMutedPitchClasses(branch, state);
  ...
  describe('prepareVoicingInput', () => {
    it('builds structure and mutes in one pass', () => {
      ...
      const { pitchStructure, mutedPitchClasses } =
        prepareVoicingInput(branch, state);
    ...
    it('matches separate structure and mute helpers', () => {
      ...
      const combined = prepareVoicingInput(branch, state);
      expect(combined.pitchStructure).toEqual(
        generatePitchStructureForVoicing(branch, state)
      );
      expect(combined.mutedPitchClasses).toEqual(
        getMutedPitchClasses(branch, state)
      );
    });
  });
  ...
  describe('applyBorrowingOverlay', () => {
    it('replaces natural pitch class at the closest MIDI octave', () => {
      ...
      const { naturalPc, effectivePc } = getVoicePitchClasses(
        branch,
        state,
        4
      );
      ...
      const overlaid = applyBorrowingOverlay(neutral, branch, state);
